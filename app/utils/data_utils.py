...
def format_analytics_data(data: Dict[str, Any], data_type: str, metrics: List[str] = None) -> Dict[str, Any]:
    """
...
def _format_page_insights(data: List[Dict], metrics: List[str] = None) -> Dict[str, Any]:
    ...
    for insight in data:
        ...
        if values:
            ...
            if insight_name == 'page_impressions':
                ...
                formatted_data["summary"]["total_fans"] = value  # Current count, not cumulative
...
def _format_ad_insights(data: List[Dict], metrics: List[str] = None) -> Dict[str, Any]:
    ...
    for insight in data:
        ...
        # Update summary
        formatted_data["summary"]["total_impressions"] += formatted_insight["impressions"]
        formatted_data["summary"]["total_clicks"] += formatted_insight["clicks"]
        formatted_data["summary"]["total_spend"] += formatted_insight["spend"]
        formatted_data["summary"]["total_conversions"] += formatted_insight["conversions"]
...
def combine_multiple_data_sources(data_sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for source_data in data_sources:
        if source_data.get("status") == "success":
            ...
            combined_data["summary"]["total_impressions"] += summary.get("total_impressions", 0)
            combined_data["summary"]["total_engagements"] += summary.get("total_engagements", 0)
