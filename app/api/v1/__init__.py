...
if os.getenv("ENVIRONMENT", "production") in ["development", "dev", "local"]:
    ...
    print(
        "⚠️ DEBUG MODE: Debug campaigners endpoint enabled at /api/v1/debug/campaigners/{id}"
    )
...
api_router.include_router(
    google_analytics_oauth.router, tags=["google-analytics-oauth"]
)
...
api_router.include_router(
    facebook_marketing_oauth.router, tags=["facebook-marketing-oauth"]
)
...
api_router.include_router(customer_analysis.router, tags=["customer-analysis"])
api_router.include_router(
...
api_router.include_router(
    customer_assignments.router, prefix="/customers", tags=["customer-assignments"]
)
...
api_router.include_router(qa.router, tags=["qa-testing"])
