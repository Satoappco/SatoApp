    ...
    try:
        with get_session() as session:
            ...
            for platform in platforms:
                platform_type = platform.asset_type.value
                if platform_type not in summary:
                    summary[platform_type] = {
                        "count": 0,
                        "active": 0,
                        "inactive": 0,
                        "providers": set(),
                ...
                summary[platform_type]["count"] += 1
                if platform.is_active:
                    summary[platform_type]["active"] += 1
                else:
                    summary[platform_type]["inactive"] += 1
                summary[platform_type]["providers"].add(platform.provider)

            # Convert sets to lists for JSON serialization
            for platform_type in summary:
                summary[platform_type]["providers"] = list(
                    summary[platform_type]["providers"]
                )
