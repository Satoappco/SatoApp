            ...
            try:
                ...
                if app_id:
                    ...
                    response = requests.delete(
                        revoke_url, params={"access_token": access_token}
                    )

                    if response.status_code == 200:
                        print(
                            f"Successfully revoked Facebook token for connection {connection_id}"
                        )
                    else:
                        print(
                            f"Warning: Facebook revocation returned status {response.status_code} for connection {connection_id}"
                        )
                else:
                    print(
                        f"Warning: FACEBOOK_APP_ID not set, cannot revoke with Facebook"
                    )

            except Exception as e:
                print(
                    f"Error revoking token with Facebook for connection {connection_id}: {str(e)}"
                )
...
async def fetch_facebook_data(
    request: FacebookDataRequest, current_user: Campaigner = Depends(get_current_user)
):
    """
...
async def get_available_facebook_pages(
    subclient_id: int, current_user: Campaigner = Depends(get_current_user)
):
    """
    ...
    try:
        with get_session() as session:
            ...
            if (
                ...
                access_token = facebook_service._decrypt_token(
                    connection.access_token_enc
                )

            print(
                f"DEBUG: Fetching all available Facebook pages for user {current_user.id}"
            )
...
async def get_available_facebook_ad_accounts(
    ...
    try:
        with get_session() as session:
            ...
            print(
                f"DEBUG: Fetching all available Facebook ad accounts for user {current_user.id}"
            )
            ...
            for ad_account in ad_accounts:
                ad_account["is_connected"] = (
                    ad_account["id"] in connected_ad_account_ids
                )
