...
from app.core.rbac import get_accessible_customers, user_can_access_customer
...
def _get_metric_weights_and_normalizer(session):
    ...
    try:
        ...
        normalizer = float(
            settings_dict.get("metric_score_normalizer", default_normalizer)
        )

        return {"weights": weights, "normalizer": normalizer}
    except Exception as e:
        logger.warning(
            f"Failed to load metric weights from settings, using defaults: {e}"
        )
...
async def get_metrics(
    ...
    start_date: Optional[date] = Query(
        None, description="Filter metrics from this date (inclusive)"
    ),
    end_date: Optional[date] = Query(
        None, description="Filter metrics to this date (inclusive)"
    ),
    platform_id: Optional[int] = Query(
        None, description="Filter by specific platform/digital asset ID"
    ),
    item_type: Optional[str] = Query(
        None, description="Filter by item type ('ad' or 'ad_group')"
    ),
    customer_id: Optional[int] = Query(
        None,
        description="Filter by customer ID (admin/campaigner can only access their assigned customers)",
    ),
):
    """
    ...
    try:
        with get_session() as session:
            ...
            # Role-based access control using centralized functions
            ...
            # If customer_id filter is provided, validate access
            if customer_id:
                ...
                accessible_customer_ids = [customer_id]

            # Get platform IDs for accessible customers
            platform_ids = session.exec(
                select(DigitalPlatform.id).where(
                    DigitalPlatform.customer_id.in_(accessible_customer_ids)
                )
            ).all()
            ...
            if conditions:
                statement = (
                    ...
                    .order_by(
                        Metrics.metric_date.desc(), Metrics.platform_id, Metrics.item_id
                    )
                    .offset(offset)
                    .limit(limit)
                )
            ...
            logger.info(
                f"[Metrics API] Returning {len(metrics)} metrics (total: {total})"
            )
...
async def get_aggregated_metrics(
    ...
    item_id: Optional[str] = Query(
        None, description="Filter by specific item ID (ad or ad_group)"
    ),
    customer_id: Optional[int] = Query(
        None,
        description="Filter by customer ID (admin/campaigner can only access their assigned customers)",
    ...
    group_by: Optional[str] = Query(
        "item_id",
        description="Group results by 'item_id' (default), 'platform_id', 'item_type', or 'none' for total aggregation",
    ),
):
    """
    ...
    try:
        with get_session() as session:
            ...
            if current_user.role == UserRole.OWNER:
                logger.info(
                    f"[Aggregated Metrics API] OWNER {current_user.id} accessing aggregated metrics"
                )
                ...
                if platform_id:
                    ...
                    if customer_platform_ids:
                        conditions.append(
                            Metrics.platform_id.in_(customer_platform_ids)
                        )
                    else:
                ...
                logger.info(
                    f"[Aggregated Metrics API] ADMIN {current_user.id} accessing aggregated metrics"
                )
                ...
                platform_ids = session.exec(
                    select(DigitalPlatform.id).where(
                        DigitalPlatform.customer_id.in_(customer_ids)
                    )
                ).all()
                ...
                logger.info(
                    f"[Aggregated Metrics API] {current_user.role.value} {current_user.id} accessing aggregated metrics"
                )

                assigned_customer_ids = session.exec(
                    select(CustomerCampaignerAssignment.customer_id).where(
                        and_(
                            CustomerCampaignerAssignment.campaigner_id
                            == current_user.id,
                ...
                platform_ids = session.exec(
                    select(DigitalPlatform.id).where(
                        DigitalPlatform.customer_id.in_(assigned_customer_ids)
                    )
                ).all()
            ...
            for row in results:
                ...
                cpa = (
                    round(total_spent / total_conversions, 2)
                ...
                cpc = (
                    round(total_spent / total_clicks, 2)
                    if total_clicks and total_clicks > 0
                    else None
                )
                cpm = (
                    round((total_spent / total_impressions) * 1000, 2)
                    if total_impressions and total_impressions > 0
                    else None
                )
                cpl = (
                    round(total_spent / total_leads, 2)
                ...
                metric = {
                    **group_info,
                    ...
                    "frequency_min": round(total_impressions / reach_max, 2)
                    if reach_max and reach_max > 0
                    else None,
                    "frequency_max": round(total_impressions / reach_min, 2)
                    if reach_min and reach_min > 0
                    else None,
                    # Performance score
                    "score": score,
                }
            ...
            logger.info(
                f"[Aggregated Metrics API] Returning {len(aggregated_metrics)} aggregated metric(s)"
            )
        ...
        logger.error(
            f"[Aggregated Metrics API] Error fetching aggregated metrics: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def _empty_aggregated_response(
    ...
    if weights and normalizer:
        notes["score"] = (
            f"Performance score (0-100) calculated using weighted metrics. Weights: CPA={weights['cpa']}, CVR={weights['cvr']}, CTR={weights['ctr']}, CPC={weights['cpc']}, CPM={weights['cpm']}, CPL={weights['cpl']}. Normalizer={normalizer}. Higher scores indicate better performance."
        )
