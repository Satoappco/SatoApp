...
def _format_kpi_value(
    kpi_settings: List[KpiSettings], kpi_type: str, index: int
) -> Optional[str]:
    """
...
class KpiGoalCreate(BaseModel):
    ...
    ad_name_headline: Optional[str] = Field(
        None, max_length=500
...
class DigitalPlatformCreate(BaseModel):
    ...
    try:
        with get_session() as session:
            ...
            statement = statement.order_by(
                KpiCatalog.subtype, KpiCatalog.primary_metric
            )
...
async def get_kpi_goals(
    ...
    try:
        with get_session() as session:
            ...
            statement = select(KpiGoal, KpiValue).join(
                KpiValue, KpiGoal.id == KpiValue.kpi_goal_id, isouter=True
            )
            ...
            statement = (
                statement.order_by(KpiGoal.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
...
async def get_kpi_goal(
    kpi_goal_id: int, current_user: Campaigner = Depends(get_current_user)
):
    ...
    try:
        with get_session() as session:
            ...
            if not campaign:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="KPI goal not found"
                )
        ...
        with get_session() as session:
            ...
            try:
                ...
                kpi_settings = session.exec(
                    select(KpiSettings).where(
                        and_(
                            ...
                            KpiSettings.campaign_objective
                            == new_campaign.campaign_objective,
                        )
                    )
                ).all()
                ...
                if kpi_settings:
                    ...
                    session.commit()
                    logger.info(
                        f"✅ Auto-created KPI Values for campaign {new_campaign.campaign_id}"
                    )
                else:
                    ...
                    session.commit()
                    logger.info(
                        f"✅ Auto-created KPI Values for campaign {new_campaign.campaign_id} (no KPI settings)"
                    )

            except Exception as e:
                logger.error(
                    f"❌ Failed to auto-create KPI Values for campaign {new_campaign.campaign_id}: {str(e)}"
                )
        ...
        with get_session() as session:
            ...
            for key, value in update_data.items():
                try:
                    ...
                    logger.error(
                        f"Failed to set attribute {key} with value {value}: {str(e)}"
                    )
                    raise
            ...
            try:
                kpi_value = session.exec(
                    select(KpiValue).where(KpiValue.kpi_goal_id == campaign.id)
                ).first()
                if kpi_value:
                    for key, value in update_data.items():
                        if key not in PROTECTED_VALUE_FIELDS and hasattr(
                            kpi_value, key
                        ):
                            ...
                            if key in [
                                ...
                                new_value = sync_kpi_string_structure(
                                    value, current_value
                                )
                ...
                logger.error(
                    f"❌ Failed to sync KPI value for goal {campaign.id}: {str(e)}"
                )
...
async def bulk_update_kpi_goals(bulk_data: KpiGoalBulkUpdateRequest):
        ...
        with get_session() as session:
            ...
            for item in items_to_create:
                try:
                    ...
                    if not item.campaign_name:
                        errors.append(
                            {"item": item.dict(), "error": "campaign_name is required"}
                        )
                        continue
            ...
            for goal_id, item in items_to_update.items():
                try:
                    ...
                    if not campaign:
                        errors.append(
                            {"item_id": goal_id, "error": "KPI goal not found"}
                        )
                        continue
                    ...
                    errors.append({"item_id": goal_id, "error": str(e)})
...
async def delete_kpi_goal(
    ...
    try:
        ...
        return {"success": True, "message": "KPI goal deleted successfully"}
...
async def get_kpi_values(
    ...
    try:
        with get_session() as session:
            ...
            statement = select(KpiValue, KpiGoal).join(
                KpiGoal, KpiValue.kpi_goal_id == KpiGoal.id, isouter=True
            )
            ...
            statement = (
                statement.order_by(KpiValue.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
            results = session.exec(statement).all()
...
async def update_kpi_value(
    ...
    try:
        with get_session() as session:
            ...
            if not campaign:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="KPI value not found"
                )
...
async def delete_kpi_value(
    ...
    try:
        with get_session() as session:
            ...
            return {"success": True, "message": "KPI value deleted successfully"}
...
async def get_kpi_subtypes():
    ...
    try:
        with get_session() as session:
            ...
            return {"success": True, "subtypes": subtypes}
...
async def get_routing_rules(
    ...
    try:
        with get_session() as session:
            ...
            statement = statement.order_by(
                RoutingRule.priority.desc(), RoutingRule.intent_pattern
            )
...
async def get_users(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(Campaigner.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
...
async def get_customers(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(Customer.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
...
@router.get("/digital-platforms")
async def get_digital_platforms(
    customer_id: Optional[int] = Query(None, description="Filter by customer ID"),
    active_only: bool = Query(False, description="Return only active platforms"),
    asset_type: Optional[str] = Query(None, description="Filter by platform type"),
    limit: int = Query(100, description="Maximum number of results"),
    offset: int = Query(0, description="Offset for pagination"),
    current_user: Campaigner = Depends(get_current_user),
):
    """Get digital platforms with optional filtering by customer"""
    try:
        with get_session() as session:
...
@router.get("/digital-platforms/{platform_id}")
async def get_digital_platform(
    platform_id: int, current_user: Campaigner = Depends(get_current_user)
):
    """Get a specific digital platform"""
    try:
        with get_session() as session:
            platform = session.get(DigitalPlatform, platform_id)

            if not platform:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="Digital platform not found",
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch digital platform: {str(e)}",
        )


@router.post("/digital-platforms")
...
@router.put("/digital-platforms/{platform_id}")
async def update_digital_platform(
    ...
    try:
        with get_session() as session:
            asset = session.get(DigitalPlatform, platform_id)
...
@router.get("/digital-platforms/{platform_id}/dependencies")
    ...
    try:
        with get_session() as session:
            ...
            connections = session.exec(
                select(Connection).where(Connection.digital_platform_id == platform_id)
            ).all()

            # Find campaign mappings that reference this asset (using raw SQL since no model exists)
            # Check campaigns table
            campaigns_result = session.execute(
                sa_text("SELECT id FROM campaigns WHERE digital_platform_id = :platform_id"),
                {"platform_id": platform_id},
            ).fetchall()

            # Check campaign_mappings table for ga4_platform_id and google_ads_platform_id
            mappings_result = session.execute(
                sa_text("""
                    SELECT id FROM campaign_mappings
                    WHERE ga4_platform_id = :platform_id OR google_ads_platform_id = :platform_id
                """),
                {"platform_id": platform_id},
            ).fetchall()
            ...
            return {
                ...
                "dependencies": {
                    ...
                    "campaigns": len(campaigns_result) if campaigns_result else 0,
                    "campaign_mappings": len(mappings_result) if mappings_result else 0,
                },
                "can_delete": True,  # All dependencies can be safely handled
            }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get asset dependencies: {str(e)}",
        )


@router.delete("/digital-platforms/{platform_id}")
async def delete_digital_platform(
    ...
    try:
        with get_session() as session:
            ...
            campaigns_count = len(campaigns_result) if campaigns_result else 0
            mappings_count = len(mappings_result) if mappings_result else 0

            # If there are campaign mappings, we need to handle them
            if mappings_count > 0:
                # Nullify references in campaign_mappings
                session.execute(
                    sa_text("""
                        UPDATE campaign_mappings
                        SET ga4_platform_id = NULL
                        WHERE ga4_platform_id = :platform_id
                    """),
                    {"platform_id": platform_id},
                )
                session.execute(
                    sa_text("""
                        UPDATE campaign_mappings
                        SET google_ads_platform_id = NULL
                        WHERE google_ads_platform_id = :platform_id
                    """),
                    {"platform_id": platform_id},
                )

            # Delete connections first (they reference the asset)
            ...
            return {
                ...
                "deleted": {
                    ...
                    "campaign_mappings_updated": mappings_count,
                    "campaigns_affected": campaigns_count,
                },
            }
...
async def get_connections(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(Connection.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
                ...
                "connections": [
                    {
                        ...
                        "rotated_at": conn.rotated_at.isoformat()
                        if conn.rotated_at
                        else None,
...
async def get_user_property_selections(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(UserPropertySelection.created_at.desc())
                .offset(offset)
                .limit(limit)
            )
...
async def get_customer_logs(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(CustomerLog.date_time.desc())
                .offset(offset)
                .limit(limit)
            )
...
async def get_detailed_execution_logs(
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                statement.order_by(DetailedExecutionLog.timestamp.desc())
                .offset(offset)
                .limit(limit)
            )
            logs = session.exec(statement).all()
...
async def get_kpi_settings(
    ...
    include_defaults: bool = Query(
        False, description="Include default settings if customer has none"
    ),
    ...
    try:
        with get_session() as session:
            ...
            statement = (
                select(KpiSettings)
                .where(KpiSettings.composite_id.like(pattern))
                .offset(offset)
                .limit(limit)
            )
            ...
            if not settings and include_defaults:
                default_statement = select(DefaultKpiSettings).order_by(
    ...
    try:
        with get_session() as session:
            ...
            composite_id = compose_id(
                customer.agency_id, current_user.id, setting_data.customer_id
            )
...
async def update_kpi_setting(
    ...
    try:
        with get_session() as session:
            ...
            return {"success": True, "message": "KPI setting updated successfully"}
...
async def delete_kpi_setting(
    ...
    try:
        with get_session() as session:
            ...
            return {"success": True, "message": "KPI setting deleted successfully"}
    ...
    try:
        with get_session() as session:
            ...
            existing_settings = session.exec(
                select(KpiSettings).where(KpiSettings.composite_id.like(pattern))
            ).all()
    ...
    return {
        ...
        "tables": [
            # Core user and customer management
            {
                "name": "campaigners",
                "label": "Campaigners",
                "description": "System campaigners",
                "endpoint": "/api/v1/database/campaigners",
            },
            {
                "name": "customers",
                "label": "Customers",
                "description": "Customer organizations",
                "endpoint": "/api/v1/database/customers",
            },
            # Digital assets and connections
            {
                ...
                "endpoint": "/api/v1/database/digital-platforms",
            },
            {
                "name": "connections",
                "label": "Connections",
            ...
            # KPI and analytics
            {
                "name": "kpi_catalog",
                "label": "KPI Catalog",
                "description": "Standardized KPI definitions",
                "endpoint": "/api/v1/database/catalog",
            },
            {
                "name": "kpi_goals",
                "label": "KPI Goals",
                "description": "KPI goal metrics",
                "endpoint": "/api/v1/database/kpi-goals",
            },
            # Agent configurations
            {
                ...
                "endpoint": "/api/v1/database/agent-configs",
            },
            {
                "name": "routing_rules",
                "label": "Routing Rules",
                "description": "Intent routing rules",
                "endpoint": "/api/v1/database/routing-rules",
            },
            # Execution logs (2-table architecture for comprehensive logging)
            {
                "name": "customer_logs",
                "label": "Customer Logs",
                "description": "Customer execution logs (business-level with timing)",
                "endpoint": "/api/v1/database/customer-logs",
            },
            {
                "name": "detailed_execution_logs",
                "label": "Detailed Execution Logs",
                "description": "Detailed execution logs (debugging)",
                "endpoint": "/api/v1/database/detailed-execution-logs",
            },
...
async def get_audiences():
    ...
    try:
        with get_session() as session:
            audiences = session.exec(
                select(Audience).order_by(Audience.audience_name)
            ).all()
    ...
    try:
        with get_session() as session:
            ...
            if (
                ...
                and audience_data.audience_name != audience.audience_name
            ):
                existing = session.exec(
                    select(Audience).where(
                        Audience.audience_name == audience_data.audience_name
