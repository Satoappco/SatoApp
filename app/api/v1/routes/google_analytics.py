...
async def save_ga_connection(
    ...
    if not user_can_access_customer(current_user, request.customer_id):
        raise HTTPException(
...
async def get_ga_connections(
    ...
    try:
        ...
        connections = await ga_service.get_user_ga_connections(
...
async def get_ga_connection(
    ...
    try:
        ...
        with get_session() as session:
            ...
            is_outdated = (
                ga_service.is_ga_token_expired(connection.expires_at)
...
async def get_reauth_url(
    ...
    try:
        ...
        reauth_url = ga_service.generate_reauth_url(
            connection_id, "satoappco@gmail.com"
        )
...
async def fetch_ga4_data(
    request: GA4DataRequest, current_user: Campaigner = Depends(get_current_user)
):
    """
    ...
    try:
        # Verify campaigner owns this connection and has access to the customer
        with get_session() as session:
            statement = select(Connection).where(
    ...
    try:
        with get_session() as session:
            ...
            if (
                ...
                print(
                    f"🔄 Google Analytics token expired or expiring soon, refreshing..."
                )
                try:
                    # Refresh the token
                    ...
                    if (
                        "Please re-authorize your Google Analytics connection:"
                        in error_message
                    ):
            ...
            print(
                f"DEBUG: Fetching all available GA4 properties for user {current_user.id}"
            )
            ...
            for account in accounts:
                ...
                for property_obj in account_properties:
                    ...
                    print(
                        f"DEBUG: Found GA property: {property_obj.display_name} (ID: {property_id})"
                    )
            ...
            return {
                ...
                "refresh_token": ga_service._decrypt_token(connection.refresh_token_enc)
