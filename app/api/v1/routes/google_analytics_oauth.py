    ...
    try:
        ...
        flow = Flow.from_client_config(
            ...
            scopes=[
                ...
                'https://www.googleapis.com/auth/adsdatahub'  # Google Ads data access
            ]
        )
    ...
    try:
        ...
        try:
            ...
            properties = [{
                ...
                'property_name': 'Demo Google Analytics Property',
                'account_id': 'demo-account-123',
                'account_name': 'Demo Account',
                'display_name': 'Demo Google Analytics Property'
            }]
...
async def create_ga_connection(request: CreateConnectionRequest):
    ...
    try:
        ...
        with get_session() as session:
            ...
            # Also create Google Ads connection if the OAuth included Google Ads scopes
            google_ads_connection = None
            try:
                from app.services.google_ads_service import GoogleAdsService
                google_ads_service = GoogleAdsService()
                
                # Create a Google Ads connection with the same account details
                google_ads_connection = await google_ads_service.save_google_ads_connection_with_account(
                    user_id=user.id,
                    subclient_id=1,  # Default subclient
                    account_id=request.account_id,
                    account_name=request.account_name,
                ...
                print(f"DEBUG: Google Ads connection created: {google_ads_connection}")
            except Exception as e:
                print(f"DEBUG: Failed to create Google Ads connection: {e}")
                # Don't fail the whole process if Google Ads connection fails
            
            return {
                "success": True,
                "message": f"Successfully connected to {request.property_name}",
