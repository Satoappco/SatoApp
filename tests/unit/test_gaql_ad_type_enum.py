...
class TestGAQLAdTypeEnum:
    ...
    def test_system_prompt_contains_ad_type_documentation(self):
        ...
        ), "Missing RESPONSIVE_SEARCH_AD documentation"
        assert (
            "NEVER use: 'SEARCH_AD'" in source
        ), "Missing warning about invalid SEARCH_AD"
        ...
        assert (
            "'SEARCH_AD' (this is INVALID" in source.lower()
            or "search_ad" in source.lower()
    ...
    def test_valid_ad_type_enums(self):
        ...
        assert (
            "RESPONSIVE_SEARCH_AD" in source
        ), "RESPONSIVE_SEARCH_AD should be documented"
        ...
        for invalid in invalid_ad_types:
            assert (
                invalid in source
            ), f"Invalid enum {invalid} should be mentioned as invalid"
    ...
    def test_gaql_query_pattern_for_search_ads(self):
        ...
        assert (
            "ad_group_ad.ad.type = 'RESPONSIVE_SEARCH_AD'" in source
            or "RESPONSIVE_SEARCH_AD" in source
...
class TestGAQLQueryGeneration:
    ...
    async def test_search_ad_query_should_use_responsive_search_ad(self):
        ...
        assert (
            "'SEARCH_AD'" not in correct_query
            or "RESPONSIVE_SEARCH_AD" in correct_query
        )
