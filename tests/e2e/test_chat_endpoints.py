...
def mock_analytics_crew():
    ...
    with patch("app.api.v1.routes.chat.AnalyticsCrewPlaceholder") as MockCrew:
        crew_instance = Mock()
...
class TestChatEndpoints:
    ...
    @pytest.mark.e2e
    def test_01_initiate_new_chat(self, test_client, auth_headers, mock_app_state, mock_campaigner):
        """Test initiating a new chat conversation"""
        with patch("app.api.v1.routes.chat.get_current_user", return_value=mock_campaigner):
            with patch("app.api.v1.routes.chat.get_app_state", return_value=mock_app_state):
                with patch("app.services.chat_trace_service.get_db_session") as mock_db:
    ...
    @pytest.mark.e2e
    def test_02_regular_message(self, test_client, auth_headers, mock_app_state, mock_campaigner):
    ...
    @pytest.mark.e2e
    def test_03_streaming_message(self, test_client, auth_headers, mock_app_state, mock_campaigner):
    ...
    @pytest.mark.e2e
    def test_04_streaming_with_use_crew(self, test_client, auth_headers, mock_app_state, mock_campaigner, mock_analytics_crew):
    ...
    @pytest.mark.e2e
    def test_05_regular_message_with_use_crew(self, test_client, auth_headers, mock_app_state, mock_campaigner, mock_analytics_crew):
